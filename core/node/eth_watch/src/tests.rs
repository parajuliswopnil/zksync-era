...
// async fn create_test_watcher(connection_pool: ConnectionPool<Core>, bnb_connection_pool: ConnectionPool<Core>) -> (EthWatch, MockEthClient) {
...
//     let bnb_connection_pool = ConnectionPool::<Core>::test_pool().await;
//     setup_db(&connection_pool).await;
//     let (mut watcher, mut client) = create_test_watcher(connection_pool.clone(), bnb_connection_pool.clone()).await;
